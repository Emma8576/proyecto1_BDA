...
.PHONY: help up down down-v build shell test-tx lab-concurrency \
	...
	p1-pg-e5-up p1-pg-e5-setup p1-pg-e5-latency p1-pg-e5-down
...
help:
	...
	@echo "  E5 PostgreSQL: make p1-pg-e5-setup && make p1-pg-e5-latency (levanta primario+réplica)"
	@echo "                 make p1-pg-e5-down para bajar el par primario/réplica"
...
p1-pg-e5-up:
	$(COMPOSE) --profile e5 up -d pg-primary
	$(COMPOSE) --profile e5 up -d pg-replica

p1-pg-e5-setup: p1-pg-e5-up
	docker exec -i ti4601-pg-primary psql -U ti4601 -d ti4601 < proyecto1/e5/schema_pg.sql
	docker exec -i ti4601-pg-primary psql -U ti4601 -d ti4601 < proyecto1/e5/seed_pg.sql
	@echo "Esperando a que la réplica aplique el seed (streaming replication)..."
	@sleep 3
	docker exec -i ti4601-pg-replica psql -U ti4601 -d ti4601 -c "SELECT count(*) FROM pedido;"

p1-pg-e5-latency: p1-pg-e5-up
	$(COMPOSE) --profile e5 run --rm app-e5 \
		python3 -u proyecto1/e5/measure_latency_pg.py --runs 50 \
		| tee evidence/mediciones_e5_pg.txt

p1-pg-e5-down:
	$(COMPOSE) --profile e5 down -v